...
### Requirement: State/props references use abstract types

**原内容：** 系统使用 `{ type: 'state', variable: 'xxx' }` 和 `{ type: 'props', variable: 'xxx' }` 格式的引用。

**新内容：** 系统使用抽象 Reference 类型 `{ _type: 'reference', prefix: 'state', variable: 'xxx' }` 或 `{ _type: 'reference', prefix: 'props', variable: 'xxx' }`。

#### Scenario: Create state with abstract reference
- **WHEN** schema.state.initial 为抽象 Reference 类型
- **THEN** 状态值通过 prefix 识别来源（state vs props）

#### Scenario: Access state reference in expression
- **WHEN** 表达式中使用 `ref_state_xxx`
- **THEN** 解析为 prefix='state' 的抽象 Reference
