## 1. 项目骨架

- [ ] 1.1 创建 package.json，配置依赖
- [ ] 1.2 创建 vite.config.ts，配置别名和插件
- [ ] 1.3 创建 tsconfig.json
- [ ] 1.4 创建 index.html
- [ ] 1.5 创建 src/main.ts
- [ ] 1.6 创建 src/App.vue
- [ ] 1.7 创建 src/setup-app.ts
- [ ] 1.8 创建 src/styles/main.css
- [ ] 1.9 创建 public/schemas/app.json（配置 plugins 和 router）
- [ ] 1.10 创建 public/schemas/app-root.json（侧边栏导航布局）

## 2. 通用组件 General

### 2.1 Button
- [ ] 2.1.1 创建 demos/general/button/basic.json
- [ ] 2.1.2 创建 demos/general/button/icon.json
- [ ] 2.1.3 创建 demos/general/button/size.json
- [ ] 2.1.4 创建 demos/general/button/loading.json
- [ ] 2.1.5 创建 demos/general/button/disabled.json
- [ ] 2.1.6 创建 demos/general/button/danger.json
- [ ] 2.1.7 创建 demos/general/button/block.json
- [ ] 2.1.8 创建 demos/general/button/ghost.json

### 2.2 Icon
- [ ] 2.2.1 创建 demos/general/icon/basic.json
- [ ] 2.2.2 创建 demos/general/icon/two-tone.json

### 2.3 Typography
- [ ] 2.3.1 创建 demos/general/typography/basic.json
- [ ] 2.3.2 创建 demos/general/typography/title.json
- [ ] 2.3.3 创建 demos/general/typography/text.json
- [ ] 2.3.4 创建 demos/general/typography/paragraph.json

## 3. 布局组件 Layout

### 3.1 Grid
- [ ] 3.1.1 创建 demos/layout/grid/basic.json
- [ ] 3.1.2 创建 demos/layout/grid/gutter.json
- [ ] 3.1.3 创建 demos/layout/grid/offset.json
- [ ] 3.1.4 创建 demos/layout/grid/sort.json

### 3.2 Layout
- [ ] 3.2.1 创建 demos/layout/layout/basic.json
- [ ] 3.2.2 创建 demos/layout/layout/top-side.json
- [ ] 3.2.3 创建 demos/layout/layout/custom-trigger.json

### 3.3 Space
- [ ] 3.3.1 创建 demos/layout/space/basic.json
- [ ] 3.3.2 创建 demos/layout/space/size.json
- [ ] 3.3.3 创建 demos/layout/space/direction.json

### 3.4 Divider
- [ ] 3.4.1 创建 demos/layout/divider/basic.json
- [ ] 3.4.2 创建 demos/layout/divider/with-text.json

## 4. 导航组件 Navigation

### 4.1 Menu
- [ ] 4.1.1 创建 demos/navigation/menu/basic.json
- [ ] 4.1.2 创建 demos/navigation/menu/inline.json
- [ ] 4.1.3 创建 demos/navigation/menu/vertical.json

### 4.2 Dropdown
- [ ] 4.2.1 创建 demos/navigation/dropdown/basic.json
- [ ] 4.2.2 创建 demos/navigation/dropdown/placement.json

### 4.3 Breadcrumb
- [ ] 4.3.1 创建 demos/navigation/breadcrumb/basic.json
- [ ] 4.3.2 创建 demos/navigation/breadcrumb/separator.json

### 4.4 Steps
- [ ] 4.4.1 创建 demos/navigation/steps/basic.json
- [ ] 4.4.2 创建 demos/navigation/steps/small.json
- [ ] 4.4.3 创建 demos/navigation/steps/icons.json

### 4.5 Pagination
- [ ] 4.5.1 创建 demos/navigation/pagination/basic.json
- [ ] 4.5.2 创建 demos/navigation/pagination/more.json
- [ ] 4.5.3 创建 demos/navigation/pagination/changer.json

## 5. 数据录入 Data Entry

### 5.1 Form
- [ ] 5.1.1 创建 demos/data-entry/form/basic.json
- [ ] 5.1.2 创建 demos/data-entry/form/layout.json
- [ ] 5.1.3 创建 demos/data-entry/form/validation.json
- [ ] 5.1.4 创建 demos/data-entry/form/dynamic.json

### 5.2 Input
- [ ] 5.2.1 创建 demos/data-entry/input/basic.json
- [ ] 5.2.2 创建 demos/data-entry/input/size.json
- [ ] 5.2.3 创建 demos/data-entry/input/addon.json
- [ ] 5.2.4 创建 demos/data-entry/input/password.json
- [ ] 5.2.5 创建 demos/data-entry/input/textarea.json

### 5.3 Select
- [ ] 5.3.1 创建 demos/data-entry/select/basic.json
- [ ] 5.3.2 创建 demos/data-entry/select/multiple.json
- [ ] 5.3.3 创建 demos/data-entry/select/search.json
- [ ] 5.3.4 创建 demos/data-entry/select/size.json

### 5.4 Checkbox
- [ ] 5.4.1 创建 demos/data-entry/checkbox/basic.json
- [ ] 5.4.2 创建 demos/data-entry/checkbox/group.json
- [ ] 5.4.3 创建 demos/data-entry/checkbox/controller.json

### 5.5 Radio
- [ ] 5.5.1 创建 demos/data-entry/radio/basic.json
- [ ] 5.5.2 创建 demos/data-entry/radio/group.json
- [ ] 5.5.3 创建 demos/data-entry/radio/button.json

### 5.6 Switch
- [ ] 5.6.1 创建 demos/data-entry/switch/basic.json
- [ ] 5.6.2 创建 demos/data-entry/switch/size.json
- [ ] 5.6.3 创建 demos/data-entry/switch/loading.json

### 5.7 DatePicker
- [ ] 5.7.1 创建 demos/data-entry/date-picker/basic.json
- [ ] 5.7.2 创建 demos/data-entry/date-picker/format.json
- [ ] 5.7.3 创建 demos/data-entry/date-picker/disabled-date.json

### 5.8 TimePicker
- [ ] 5.8.1 创建 demos/data-entry/time-picker/basic.json
- [ ] 5.8.2 创建 demos/data-entry/time-picker/format.json

### 5.9 Upload
- [ ] 5.9.1 创建 demos/data-entry/upload/basic.json
- [ ] 5.9.2 创建 demos/data-entry/upload/drag.json

### 5.10 Rate
- [ ] 5.10.1 创建 demos/data-entry/rate/basic.json
- [ ] 5.10.2 创建 demos/data-entry/rate/half.json

### 5.11 Slider
- [ ] 5.11.1 创建 demos/data-entry/slider/basic.json
- [ ] 5.11.2 创建 demos/data-entry/slider/marks.json

## 6. 数据展示 Data Display

### 6.1 Table
- [ ] 6.1.1 创建 demos/data-display/table/basic.json
- [ ] 6.1.2 创建 demos/data-display/table/pagination.json
- [ ] 6.1.3 创建 demos/data-display/table/selection.json
- [ ] 6.1.4 创建 demos/data-display/table/sort-filter.json

### 6.2 Modal
- [ ] 6.2.1 创建 demos/data-display/modal/basic.json
- [ ] 6.2.2 创建 demos/data-display/modal/position.json
- [ ] 6.2.3 创建 demos/data-display/modal/confirm.json
- [ ] 6.2.4 创建 demos/data-display/modal/async.json

### 6.3 Tabs
- [ ] 6.3.1 创建 demos/data-display/tabs/basic.json
- [ ] 6.3.2 创建 demos/data-display/tabs/position.json
- [ ] 6.3.3 创建 demos/data-display/tabs/card.json

### 6.4 List
- [ ] 6.4.1 创建 demos/data-display/list/basic.json
- [ ] 6.4.2 创建 demos/data-display/list/vertical.json
- [ ] 6.4.3 创建 demos/data-display/list/grid.json

### 6.5 Card
- [ ] 6.5.1 创建 demos/data-display/card/basic.json
- [ ] 6.5.2 创建 demos/data-display/card/meta.json
- [ ] 6.5.3 创建 demos/data-display/card/grid.json

### 6.6 Tree
- [ ] 6.6.1 创建 demos/data-display/tree/basic.json
- [ ] 6.6.2 创建 demos/data-display/tree/directory.json
- [ ] 6.6.3 创建 demos/data-display/tree/search.json

### 6.7 Descriptions
- [ ] 6.7.1 创建 demos/data-display/descriptions/basic.json
- [ ] 6.7.2 创建 demos/data-display/descriptions/border.json

### 6.8 Avatar
- [ ] 6.8.1 创建 demos/data-display/avatar/basic.json
- [ ] 6.8.2 创建 demos/data-display/avatar/type.json
- [ ] 6.8.3 创建 demos/data-display/avatar/group.json

### 6.9 Badge
- [ ] 6.9.1 创建 demos/data-display/badge/basic.json
- [ ] 6.9.2 创建 demos/data-display/badge/no-wrapper.json
- [ ] 6.9.3 创建 demos/data-display/badge/status.json

### 6.10 Tag
- [ ] 6.10.1 创建 demos/data-display/tag/basic.json
- [ ] 6.10.2 创建 demos/data-display/tag/colorful.json
- [ ] 6.10.3 创建 demos/data-display/tag/checkable.json

## 7. 反馈组件 Feedback

### 7.1 Alert
- [ ] 7.1.1 创建 demos/feedback/alert/basic.json
- [ ] 7.1.2 创建 demos/feedback/alert/closable.json
- [ ] 7.1.3 创建 demos/feedback/alert/description.json

### 7.2 Message
- [ ] 7.2.1 创建 demos/feedback/message/basic.json
- [ ] 7.2.2 创建 demos/feedback/message/type.json
- [ ] 7.2.3 创建 demos/feedback/message/loading.json

### 7.3 Notification
- [ ] 7.3.1 创建 demos/feedback/notification/basic.json
- [ ] 7.3.2 创建 demos/feedback/notification/placement.json
- [ ] 7.3.3 创建 demos/feedback/notification/type.json

### 7.4 Progress
- [ ] 7.4.1 创建 demos/feedback/progress/basic.json
- [ ] 7.4.2 创建 demos/feedback/progress/circle.json
- [ ] 7.4.3 创建 demos/feedback/progress/dynamic.json

### 7.5 Spin
- [ ] 7.5.1 创建 demos/feedback/spin/basic.json
- [ ] 7.5.2 创建 demos/feedback/spin/size.json
- [ ] 7.5.3 创建 demos/feedback/spin/tip.json

### 7.6 Skeleton
- [ ] 7.6.1 创建 demos/feedback/skeleton/basic.json
- [ ] 7.6.2 创建 demos/feedback/skeleton/complex.json

## 8. 其他组件 Other

### 8.1 Affix
- [ ] 8.1.1 创建 demos/other/affix/basic.json
- [ ] 8.1.2 创建 demos/other/affix/bottom.json

### 8.2 Anchor
- [ ] 8.2.1 创建 demos/other/anchor/basic.json
- [ ] 8.2.2 创建 demos/other/anchor/horizontal.json

### 8.3 BackTop
- [ ] 8.3.1 创建 demos/other/back-top/basic.json
- [ ] 8.3.2 创建 demos/other/back-top/custom.json

### 8.4 Collapse
- [ ] 8.4.1 创建 demos/other/collapse/basic.json
- [ ] 8.4.2 创建 demos/other/collapse/accordion.json
- [ ] 8.4.3 创建 demos/other/collapse/mix.json

### 8.5 Calendar
- [ ] 8.5.1 创建 demos/other/calendar/basic.json

### 8.6 Drawer
- [ ] 8.6.1 创建 demos/other/drawer/basic.json
- [ ] 8.6.2 创建 demos/other/drawer/placement.json

### 8.7 Empty
- [ ] 8.7.1 创建 demos/other/empty/basic.json
- [ ] 8.7.2 创建 demos/other/empty/custom.json

### 8.8 Popover
- [ ] 8.8.1 创建 demos/other/popover/basic.json
- [ ] 8.8.2 创建 demos/other/popover/trigger.json

### 8
