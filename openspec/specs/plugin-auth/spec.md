...
- **WHEN** 调用 `$_core_auth.has("user:read")`
- **THEN** 返回是否有该权限

#### Scenario: Check any permission
- **WHEN** 调用 `$_core_auth.hasAny(["user:read", "user:write"])`
- **THEN** 返回是否有任一权限

#### Scenario: Check all permissions
- **WHEN** 调用 `$_core_auth.hasAll(["user:read", "user:write"])`
- **THEN** 返回是否有所有权限

#### Scenario: Check role
- **WHEN** 调用 `$_core_auth.hasRole("admin")`
- **THEN** 返回是否有该角色
...
- **WHEN** 调用 `$_core_auth.canReadField("user", "email")`
- **THEN** 返回是否可读

#### Scenario: Check field write permission
- **WHEN** 调用 `$_core_auth.canWriteField("user", "email")`
- **THEN** 返回是否可写

#### Scenario: Check field hidden
- **WHEN** 调用 `$_core_auth.isFieldHidden("user", "salary")`
- **THEN** 返回是否隐藏
