...
describe('parseState', () => {
  ...
  it('should parse empty state definition', () => {
    const result = parseState({}, mockContext);
  ...
  it('should parse ref state type', () => {
    ...
    const result = parseState(state, mockContext);
  ...
  it('should parse state with initial expression', () => {
    const state: StateDefinition = {
      doubled: { type: 'ref', initial: '{{count * 2}}' },
    };
  ...
  it('should handle unknown state type without adding error', () => {
    const state: StateDefinition = {
      unknown: { type: 'unknownType' } as any,
    };

    parseState(state, mockContext);
