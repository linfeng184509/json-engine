...
export function parseState(definition, context) {
    const result = {};
    for (const [stateName, stateDef] of Object.entries(definition)) {
        try {
            if (typeof stateDef !== 'object' || stateDef === null) {
                throw createValidationError(`state.${stateName}`, 'State definition must be an object', 'object', stateDef);
            }
            const def = stateDef;
            if (!def.type) {
            ...
            if (def.type && !validTypes.includes(def.type)) {
                throw createValidationError(`state.${stateName}.type`, `Invalid state type: "${def.type}". Must be one of: ${validTypes.join(', ')}`, validTypes.join(' | '), def.type);
            }
            if (def.initial !== undefined && !isInitialValue(def.initial)) {
                throw createValidationError(`state.${stateName}.initial`, 'Initial value must be a literal or structured value', 'InitialValue', def.initial);
            }
            result[stateName] = {
                type: def.type || 'ref',
                initial: def.initial,
